...
async def m_cb(b, cb):
    ...
    if type_ == 'pause':
        ...
        now_playing = temp[0][0]
        by = temp[0][1].mention(style='md')
        msg = "**Lagu Yang Sedang dimainkan** di VCG {}".format(cb.message.chat.title)
        msg += "\n⎆ "+ now_playing
        msg += "\n⎆ Request dari "+by
        temp.pop(0)
        if temp:
...
async def play(_, message: Message):
                      ...
                      if administrator == message.from_user.id:  
                          ...
                          try:
                              ...
                              await lel.edit(
                                  ...
                                  "\n\nAtau tambahkan secara manual @MightyMusicV2_bot ke Grup Anda dan coba lagi</b>",
                              )
                              pass
    try:
        chatdetails = await USER.get_chat(chid)
    ...
    if message.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        photo="final.png",
        reply_markup=keyboard,
        caption="🎼️ **__Sedang Memutar Lagu tersebut, permintaan dari {}__**".format(
